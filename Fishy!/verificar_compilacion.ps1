...
$editorCsproj = ".\Assembly-CSharp-Editor.csproj"
$fuentesEditor = @()
if (Test-Path $editorCsproj) {
    $xmlEd = [xml](Get-Content $editorCsproj -Raw)
    $fuentesEditor = @($xmlEd.Project.ItemGroup.Compile | Where-Object { $_ } | ForEach-Object { $_.Include })
}
$conocidos = @($fuentes + $fuentesEditor)
...
$salida = Join-Path $env:TEMP "fishy_compilacion.dll"
$rsp    = Join-Path $env:TEMP "fishy_compilacion.rsp"

$lineas = New-Object System.Collections.Generic.List[string]
$lineas.Add("-target:library")
$lineas.Add("-langversion:$lang")
$lineas.Add("-nostdlib+")
$lineas.Add("-out:`"$salida`"")
$lineas.Add("-define:$defines")
foreach ($r in $refsOk)  { $lineas.Add("-r:`"$r`"") }
foreach ($f in $fuentes) { $lineas.Add("`"$f`"") }
[System.IO.File]::WriteAllLines($rsp, $lineas, (New-Object System.Text.UTF8Encoding $false))

$resultado = & $dotnet $csc "@$rsp"
$codigo = $LASTEXITCODE
...
if ($sinListar.Count -gt 0) {
    ...
    Escribir-Ambar "  asi que NO se compilaron. Abre Unity una vez para que lo regenere:"
    $sinListar | ForEach-Object { Escribir-Ambar "    $_" }
}
