...
export default function RootLayout() {
  const { autenticado, perfil, salir, modoDemo, esProfesor } = useSesion()
  ...
  return <div className={autenticado ? 'app-shell' : 'login-shell'}>
    ...
    {autenticado && <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        ...
        <div className="account"><span className="avatar small">{perfil?.nombre.slice(0, 1)}</span><div><strong>{perfil?.nombre}</strong><span>{esProfesor ? 'Tutor administrador' : 'Tutor padre/madre'}</span></div><button type="button" className="icon-button" onClick={salir} aria-label="Cerrar sesión" title="Cerrar sesión"><Icono nombre="salir" /></button></div>
      </div>
    </aside>}
    <div className="app-content">
      ...
      <main id="contenido" ref={main} tabIndex={-1} className={autenticado ? 'main-content' : 'login-main'}>
        <Outlet key={String(perfil?.id ?? 'guest') + ':' + modoDemo + ':' + esProfesor + ':' + pathname} />
      </main>
