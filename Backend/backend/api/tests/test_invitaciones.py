...
class InvitacionesTests(TestCase):
    ...
    def test_cuenta_nueva_se_registra_inicia_sesion_y_crea_solo_el_nino_invitado(self):
        ...
        respuesta = self.client.post("/api/auth/registro/", {"nombre": "familia_nueva", "email": "NUEVA@example.com", "password": "clave-larga", "is_admin": True, "rol": "profesor"}, format="json")
        self.assertEqual(respuesta.status_code, 201)
        nueva = AdultoResponsable.objects.get(pk=respuesta.data["adulto_id"])
        self.assertFalse(nueva.is_admin)
        self.assertEqual(nueva.rol, "padre")
    ...
    def test_administrador_tecnico_no_gestiona_grupos_por_la_api(self):
        # `is_admin` solo abre /admin/: no convierte a nadie en profesor.
        tecnico = AdultoResponsable.objects.create_superuser("equipo", "equipo@example.com", "clave")
        self.client.force_authenticate(tecnico)
        self.assertEqual(self.client.get("/api/grupos/").status_code, 403)
        self.assertEqual(self.client.post("/api/grupos/", {"nombre": "Curso"}, format="json").status_code, 403)
        self.assertEqual(self.client.get(self.base).status_code, 403)
        self.assertEqual(self.client.get(self.base + "reporte/").status_code, 403)
        self.assertEqual(self.client.get(self.base + "seguimiento/").status_code, 403)
        self.assertEqual(GrupoTutor.objects.count(), 1)

    def test_admin_de_django_muestra_todos_los_grupos_al_equipo_tecnico(self):
        # Antes filtraba por `tutor=request.user`; con los roles separados quedaba vacío.
        GrupoTutor.objects.create(tutor=self.otro_profe, nombre="6° Básico B")
        _, token = self.invitar()
        tecnico = AdultoResponsable.objects.create_superuser("equipo", "equipo@example.com", "clave")
        web = self.client_class()
        web.force_login(tecnico)
        listado = web.get("/admin/api/grupotutor/")
        self.assertEqual(listado.status_code, 200)
        self.assertContains(listado, "5° Básico A")
        self.assertContains(listado, "6° Básico B")
        invitaciones = web.get("/admin/api/invitaciongrupo/")
        self.assertContains(invitaciones, "Martina")
        self.assertNotContains(invitaciones, token)
        self.assertNotContains(invitaciones, InvitacionGrupo.objects.get().token_hash)
        # Solo lectura: ni crear ni editar desde el admin.
        self.assertEqual(web.get("/admin/api/grupotutor/add/").status_code, 403)
        self.assertEqual(web.post(f"/admin/api/grupotutor/{self.grupo.pk}/delete/", {"post": "yes"}).status_code, 403)
