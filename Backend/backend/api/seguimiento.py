...
from .reportes_web import TEMAS, TIPOS, ZONAS_BD, sin_cache, tema_portal
...
def seguimiento_grupo(request, grupo_id):
    ...
    if ids:
        ...
        # El banco guarda `reto_viral`; se lee con ambos slugs y se agrupa con el del portal.
        opciones = list(OpcionBanco.objects.filter(tipo__in=TIPOS, pregunta__zona__in=ZONAS_BD)
                        .values_list("opcion_id", "tipo", "pregunta__zona"))
        for tema in TEMAS:
            tipos = {oid: tipo for oid, tipo, zona in opciones if tema_portal(zona) == tema}
            if not tipos:
                continue
